...
export class CardWithdrawalService {
  ...
  private static async findExistingWithdrawalStrict(
    ...
    if (result) {
      ...
      allMatches.forEach((t: any, i: number) => {
  ...
  /**
   * 引き落とし取引を作成または更新（upsert）- 年月ベースの重複防止対応
   */
  private static async createWithdrawal(
    ...
    // 年月を計算（新機能）
    const yearMonth = `${withdrawalDate.getFullYear()}-${String(withdrawalDate.getMonth() + 1).padStart(2, '0')}`;

    // 新しい年月ベースのupsert処理
    await tx.transaction.upsert({
      where: {
        // 新しいユニーク制約を使用
        unique_auto_withdrawal_monthly: {
          userId,
          yearMonth,
          paymentMethodId: withdrawalPaymentMethod.id,
          store: store,
      ...
      update: {
        // 既存のレコードがある場合
        // 日付がNULLまたは未設定の場合のみ日付を更新、既存の日付は保持
        amount, // 金額は常に更新
        purpose,
        dayOfWeek,
        updatedAt: new Date(),
      ...
      console.log(`⚠️ 年月ベースのupsertが失敗、フォールバック処理を実行: ${error.message}`);
      
      // フォールバック：既存のレコードを手動で検索
      ...
      if (existing) {
        ...
        console.log(`✅ 既存レコード更新完了（日付保持）: ${existing.date}`);
      } else {
        // 新規作成
        ...
        console.log(`✅ 新規レコード作成完了: ${withdrawalDate}`);
      }
    });
